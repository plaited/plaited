 ...
 * @internal
 */
const checkModnetField = async (
	cwd: string,
	expectedMss?: z.infer<typeof MssSchema>,
): Promise<GradeCheck> => {
	...
	const patterns: Array<{ keywords: string[]; codeIndicators: string[] }> = [
		{
			...
			codeIndicators: ['<form', '<input', 'onsubmit', 'onchange', 'oninput', 'handlesubmit', 'formdata', 'addeventlistener'],
		},
		{
		...
		},
		{
			...
			codeIndicators: ['<svg', '<canvas', 'bindingcontext', 'binddata', 'bindgroup', 'bindnode', 'bindattr', 'bindprop', 'bindtext'],
		},
		{
