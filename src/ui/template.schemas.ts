...
import type { JsonObject } from '../shared/shared.schemas.ts'
...
export const TemplateObjectSchema = z.object({
  ...
  scale: z.enum([...Object.values(SCALE)]),
...
/** Schema for valid hyperscript child primitives. */
export const ChildSchema = z.union([z.number(), z.string(), TemplateObjectSchema])
...
/** Schema for hyperscript children — a single child or array of children. */
...
export const PlaitedAttributesSchema = z.object({
  ...
  children: ChildrenSchema.nullable(),
  [P_TARGET]: z.union([z.string(), z.number()]).nullable(),
  [P_TRIGGER]: z.record(z.string(), z.string()).nullable(),
  [P_SCALE]: z.enum(Object.values(SCALE)).nullable(),
  stylesheets: z.array(z.string()).nullable(),
  classNames: z.array(z.string()).nullable(),
 ...
 */
export type DetailedHTMLAttributes = HTMLAttributes & JsonObject
...
export const isCustomElementTag = (tag: unknown): tag is CustomElementTag => {
  return (
    isTypeOf<string>(tag, 'string') && CUSTOM_ELEMENT_TAG_PATTERN.test(tag) && !RESERVED_CUSTOM_ELEMENT_TAGS.has(tag)
  )
}
