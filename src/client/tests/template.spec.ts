...
import type { StylesObject, TemplateObject } from 'plaited/client'
import {
  $case,
  $default,
  $for,
...
// ── `styles` attribute tests ────────────────────────────────────

test('h: styles attribute accumulates classNames and stylesheets from a StylesObject', () => {
  const styleObj: StylesObject = { classNames: ['btn'], stylesheets: ['.btn { color: blue; }'] }
  const result = h('button', { styles: [styleObj], children: 'Click' })
  const html = result.html.join('')
  expect(html).toContain('class="btn"')
  expect(result.stylesheets).toContain('.btn { color: blue; }')
})

test('h: styles attribute concatenates multiple StylesObjects in order', () => {
  const a = { classNames: ['card'], stylesheets: ['.card { padding: 1rem; }'] }
  const b = { classNames: ['tone-danger'], stylesheets: ['.tone-danger { color: red; }'] }
  const result = h('div', { styles: [a, b], children: 'content' })
  const html = result.html.join('')
  expect(html).toContain('class="card tone-danger"')
  expect(result.stylesheets).toEqual(['.card { padding: 1rem; }', '.tone-danger { color: red; }'])
})

test('h: styles attribute skips falsy entries in the array', () => {
  const a = { classNames: ['card'], stylesheets: ['.card { padding: 1rem; }'] }
  const result = h('div', { styles: [a, undefined, false], children: 'content' })
  const html = result.html.join('')
  expect(html).toContain('class="card"')
  expect(result.stylesheets).toEqual(['.card { padding: 1rem; }'])
})

test('h: styles attribute composes alongside class and classNames', () => {
  const styleObj = { classNames: ['from-styles'], stylesheets: [] }
  const result = h('div', {
    class: 'from-class',
  ...
  expect(html).toContain('from-styles')
  expect(html).toContain('from-class')
  expect(html).toContain('from-classNames')
})
