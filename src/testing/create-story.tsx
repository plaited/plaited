...
export const createStory = <T extends Attrs = Attrs>({ args, template, play, ...rest }: StoryObj<T>) => {
  const attrs: T = args ? (args as T) : ({} as T)
  /**
   ...
   * ```
   */
  const PlaitedFixture = defineElement<{
    ...
    bProgram({ bThreads, host, bThread, bSync }) {
      ...
      return {
        async [PLAY_EVENT]({ route, entry, exportName }) {
          try {
            await play?.({
              assert,
              findByAttribute,
  ...
  return {
    template: <PlaitedFixture>{template?.(attrs)}</PlaitedFixture>,
    ...rest,
  }
}
