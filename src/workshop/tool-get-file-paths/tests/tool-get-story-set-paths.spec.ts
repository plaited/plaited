...
import { toolGetStorySetPaths } from '../tool-get-story-set-paths.js'
...
test('toolGetStorySetPaths registers tool correctly', () => {
  const server = getMcpServer()
  const cwd = '/test/path'

  // Register the tool - should not throw on first registration
  expect(() => toolGetStorySetPaths(server, cwd)).not.toThrow()

  // Attempting to register again should throw (tool already registered)
  expect(() => toolGetStorySetPaths(server, cwd)).toThrow('Tool get-story-set-paths is already registered')
})
