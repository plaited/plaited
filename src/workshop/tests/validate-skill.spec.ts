...
import { findSkillDirectories, validateSkill, validateSkills } from '../validate-skill.ts'
...
metadata:
  ...
  version: "1.0"`,
    ...
    test('reports error for name exceeding 64 characters', async () => {
      ...
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.includes('64 character limit'))).toBe(true)
    })
    ...
    test('reports error for description exceeding 1024 characters', async () => {
      ...
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.includes('1024 character limit'))).toBe(true)
    })
    ...
    test('reports error for name starting with hyphen', async () => {
      // Create directory with underscore (allowed) but name with hyphen
    ...
    test('reports error for name not matching directory', async () => {
      ...
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.includes('must match skill name'))).toBe(true)
    })
    ...
    test('reports error for non-existent directory', async () => {
      const result = await validateSkill(join(tempDir, 'nonexistent'))
    ...
    test('reports error for compatibility exceeding 500 characters', async () => {
      ...
      expect(result.valid).toBe(false)
      expect(result.errors.some((e) => e.includes('500 character limit'))).toBe(true)
    })
  })

  describe('findSkillDirectories', () => {
    test('discovers all skill directories', async () => {
      const skillsDir = join(tempDir, 'skills')
      await Bun.$`mkdir -p ${skillsDir}`.quiet()

      await createSkill('skills/skill-a', 'name: skill-a\ndescription: Skill A')
      await createSkill('skills/skill-b', 'name: skill-b\ndescription: Skill B')

      const dirs = await findSkillDirectories(skillsDir)

      expect(dirs).toHaveLength(2)
      expect(dirs.some((d) => d.endsWith('skill-a'))).toBe(true)
      expect(dirs.some((d) => d.endsWith('skill-b'))).toBe(true)
    })
  })

  describe('validateSkills', () => {
    test('validates multiple skills', async () => {
      ...
      const results = await validateSkills(multiDir)
    ...
    test('reports mixed valid and invalid skills', async () => {
      ...
      const results = await validateSkills(mixedDir)
  ...
  describe('existing Plaited skills', () => {
    test('validates all .claude/skills pass', async () => {
      const cwd = process.cwd()
      const skillsDir = join(cwd, '.claude/skills')

      const results = await validateSkills(skillsDir)
