...
export const AutoresearchFactoryModeSchema = z.enum(['off', 'observe', 'candidate', 'promote'])

export const ImprovementTargetSchema = z.object({
  kind: z.enum(['skill', 'factory', 'prompt-pack', 'search-policy', 'verification-policy']),
  id: z.string().min(1),
...
export const AutoresearchFactoryPolicySchema = z.object({
  mode: AutoresearchFactoryModeSchema,
