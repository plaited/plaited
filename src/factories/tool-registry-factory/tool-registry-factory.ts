...
import { TOOL_REGISTRY_FACTORY_EVENTS, TOOL_REGISTRY_FACTORY_SIGNAL_KEYS } from './tool-registry-factory.constants.ts'
import {
  type CapabilityRecord,
  CapabilityRecordSchema,
  CapabilityRegistrySchema,
} from './tool-registry-factory.schemas.ts'
import type { CreateToolRegistryFactoryOptions } from './tool-registry-factory.types.ts'
...
const buildRegistry = ({
  ...
  modules?: FactoryModuleCatalogEntry[]
}): CapabilityRecord[] => {
  ...
  for (const module of modules) {
    records.push(
      CapabilityRecordSchema.parse({
        id: `module:${module.id}`,
        name: module.id,
        description: `Factory module discovered at ${module.path}`,
        capabilityClass: 'module',
        sourceClass: module.sourceClass,
        path: module.path,
        tags: ['module', 'factory'],
        authorityHints: [`factory-count:${module.factoryCount}`],
      }),
    )
  }
...
/**
 * Creates the metadata-first tool registry factory.
 *
 * @public
 */
export const createToolRegistryFactory =
  ({
    registrySignalKey = TOOL_REGISTRY_FACTORY_SIGNAL_KEYS.registry,
  ...
  }: CreateToolRegistryFactoryOptions = {}): Factory =>
  ({ signals, trigger }) => {
    ...
    const rebuildRegistry = () => {
      ...
      const modules = (modulesSignal?.get() ?? []) as FactoryModuleCatalogEntry[]
      const registry = buildRegistry({ skills, modules })
      registrySignal.set?.(registry)
      trigger({
        type: TOOL_REGISTRY_FACTORY_EVENTS.tool_registry_updated,
        detail: {
