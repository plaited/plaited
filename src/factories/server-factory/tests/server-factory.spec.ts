...
describe('createServerFactory', () => {
  ...
  test('recovers after removing a conflicting contribution and reloading', async () => {
    let statusSignal!: Signal<typeof ServerFactoryStatusSchema>
    let configSignal!: Signal<typeof ServerFactoryConfigSchema>
    ...
    const agent = await createAgent({
      id: 'agent:test-server-factory-conflict-recovery',
      cwd: process.cwd(),
      workspace: process.cwd(),
      models: TEST_MODELS,
      factories: [
    ...
    expect(statusSignal.get()?.state).toBe('error')
  ...
  test('reload starts the server when it is currently stopped', async () => {
    let statusSignal!: Signal<typeof ServerFactoryStatusSchema>
    ...
    const agent = await createAgent({
      id: 'agent:test-server-factory-reload-start',
      cwd: process.cwd(),
      workspace: process.cwd(),
      models: TEST_MODELS,
      factories: [
    ...
    expect(statusSignal.get()?.state).toBe('stopped')
  ...
  test('emits server_error and stays in error state when startup fails', async () => {
    let receivedError: { message: string } | undefined
    let statusSignal!: Signal<typeof ServerFactoryStatusSchema>
    ...
    const observeServerFactory: Factory = ({ signals }) => {
      ...
      return {
        handlers: {
          [SERVER_FACTORY_EVENTS.server_error](detail) {
            receivedError = detail as { message: string }
          },
        },
      }
    }
    ...
    const agent = await createAgent({
      id: 'agent:test-server-factory-start-error',
      cwd: process.cwd(),
      workspace: process.cwd(),
      models: TEST_MODELS,
      factories: [
    ...
    expect(statusSignal.get()?.state).toBe('error')
    expect(statusSignal.get()?.error).toContain('authenticateConnection')
    expect(receivedError?.message).toContain('authenticateConnection')
  })
})
