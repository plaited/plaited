...
import {
  ...
  type ServerFactoryStatus,
  ServerFactoryStatusSchema,
...
/**
 * Creates a behavioral factory lane that owns a live server instance.
 *
 * @remarks
 * The live `Bun.serve()` instance is held privately in the factory closure and
 * controlled through behavioral events plus signal-backed config/status.
 *
 * Other factories can contribute config by writing to the config signal, and
 * can operate the live lane by triggering `SERVER_FACTORY_EVENTS`.
 *
 * @public
 */
export const createServerFactory =
  ({
    configSignalKey = SERVER_FACTORY_SIGNAL_KEYS.config,
    statusSignalKey = SERVER_FACTORY_SIGNAL_KEYS.status,
    initialConfig,
  }: CreateServerFactoryOptions = {}): Factory =>
  ({ signals, trigger }) => {
    const configSignal =
    ...
    const statusSignal =
      ...
      signals.set({
        key: statusSignalKey,
        schema: ServerFactoryStatusSchema,
        value: { state: 'stopped' },
        readOnly: true,
      })

    let currentConfig = ServerFactoryConfigSchema.parse(configSignal.get() ?? {})
    let liveServer: ServerHandle | undefined

    const publishStatus = (status: ServerFactoryStatus) => {
    ...
    const startServer = () => {
      ...
      try {
        ...
        trigger({
          type: SERVER_FACTORY_EVENTS.server_started,
    ...
    configSignal.listen(SERVER_FACTORY_EVENTS.server_config_updated)

    currentConfig = ServerFactoryConfigSchema.parse(configSignal.get() ?? {})
    ...
    return {
      handlers: {
        [SERVER_FACTORY_EVENTS.server_set_config](detail) {
          const parsed = ServerFactoryConfigSchema.safeParse(detail)
          if (!parsed.success) return
          configSignal.set?.(parsed.data)
        },
        [SERVER_FACTORY_EVENTS.server_config_updated]() {
          currentConfig = ServerFactoryConfigSchema.parse(configSignal.get() ?? {})
          if (currentConfig.autostart) {
            startServer()
          }
        },
        [SERVER_FACTORY_EVENTS.server_start]() {
          startServer()
        },
        [SERVER_FACTORY_EVENTS.server_stop]() {
          stopServer()
        },
        [SERVER_FACTORY_EVENTS.server_reload]() {
          startServer()
        },
        [SERVER_FACTORY_EVENTS.server_send](detail) {
