...
} from './node-auth-factory.schemas.ts'
import type { CreateNodeAuthFactoryOptions } from './node-auth-factory.types.ts'
...
/**
 * Creates the bounded node auth policy factory.
 *
 * @public
 */
export const createNodeAuthFactory =
  ({
    stateSignalKey = NODE_AUTH_FACTORY_SIGNAL_KEYS.state,
    threeAxisSignalKey = THREE_AXIS_FACTORY_SIGNAL_KEYS.state,
    initialMode = 'dev',
  }: CreateNodeAuthFactoryOptions = {}): Factory =>
  ({ signals, trigger }) => {
    const stateSignal =
    ...
    const publish = (next: NodeAuthState) => {
      ...
      trigger({
        type: NODE_AUTH_FACTORY_EVENTS.node_auth_factory_updated,
        detail: {
          mode: parsed.mode,
    ...
    return {
      handlers: {
        [NODE_AUTH_FACTORY_EVENTS.node_auth_factory_set_mode](detail) {
        ...
        [NODE_AUTH_FACTORY_EVENTS.node_auth_factory_authenticate](detail) {
        ...
        [NODE_AUTH_FACTORY_EVENTS.node_auth_factory_clear_session]() {
          const current = (stateSignal.get() ?? null) as NodeAuthState | null
          if (!current) return
          publish({
            ...current,
