...
import type { MemoryEpisode } from '../memory-module/memory-module.schemas.ts'
import { SESSION_PERSISTENCE_MODULE_SIGNAL_KEYS } from '../session-persistence-module/session-persistence-module.constants.ts'
import type { SessionPersistenceState } from '../session-persistence-module/session-persistence-module.schemas.ts'
...
import {
  ...
  type NodeHomeState,
  NodeHomeStateSchema,
} from './node-home-module.schemas.ts'
import type { CreateNodeHomeModuleOptions } from './node-home-module.types.ts'
...
/**
 * Creates the bounded node-home policy module.
 *
 * @public
 */
export const createNodeHomeModule =
  ({
    stateSignalKey = NODE_HOME_MODULE_SIGNAL_KEYS.state,
    sessionPersistenceSignalKey = SESSION_PERSISTENCE_MODULE_SIGNAL_KEYS.state,
    memoryEpisodesSignalKey = MEMORY_MODULE_SIGNAL_KEYS.episodes,
    editStateSignalKey = EDIT_MODULE_SIGNAL_KEYS.state,
    maxArtifacts = 12,
    ownerHost = 'local',
  }: CreateNodeHomeModuleOptions = {}): Module =>
  ({ signals, trigger }) => {
    const stateSignal =
    ...
    const publish = (next: NodeHomeState) => {
      ...
      trigger({
        type: NODE_HOME_MODULE_EVENTS.node_home_module_updated,
        detail: {
    ...
    return {
      handlers: {
        [NODE_HOME_MODULE_EVENTS.node_home_module_checkpoint](detail) {
        ...
        [NODE_HOME_MODULE_EVENTS.node_home_module_export](detail) {
        ...
        [NODE_HOME_MODULE_EVENTS.node_home_module_import](detail) {
        ...
        [NODE_HOME_MODULE_EVENTS.node_home_module_handoff](detail) {
        ...
        [NODE_HOME_MODULE_EVENTS.node_home_module_restore](detail) {
