...
import type { MemoryObservation } from '../memory-module/memory-module.schemas.ts'
...
import {
  SESSION_PERSISTENCE_MODULE_EVENTS,
  SESSION_PERSISTENCE_MODULE_SIGNAL_KEYS,
} from './session-persistence-module.constants.ts'
import {
  type SessionArtifact,
  SessionArtifactSchema,
  type SessionPersistenceState,
  SessionPersistenceStateSchema,
} from './session-persistence-module.schemas.ts'
import type { CreateSessionPersistenceModuleOptions } from './session-persistence-module.types.ts'

/**
 * Creates the bounded session persistence module.
 *
 * @public
 */
export const createSessionPersistenceModule =
  ({
    stateSignalKey = SESSION_PERSISTENCE_MODULE_SIGNAL_KEYS.state,
    workingMemorySignalKey = MEMORY_MODULE_SIGNAL_KEYS.working,
    tracesSignalKey = OBSERVABILITY_MODULE_SIGNAL_KEYS.traces,
    maxArtifacts = 10,
  }: CreateSessionPersistenceModuleOptions = {}): Module =>
  ({ signals, trigger }) => {
    const stateSignal =
    ...
    const appendArtifact = (artifact: SessionArtifact) => {
      ...
      trigger({
        type: SESSION_PERSISTENCE_MODULE_EVENTS.session_persistence_module_updated,
        detail: {
