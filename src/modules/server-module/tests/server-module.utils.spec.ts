...
import { ServerModuleConfigSchema } from '../server-module.schemas.ts'
import { formatRouteConflict, mergeRoutes } from '../server-module.utils.ts'
...
describe('ServerModuleConfigSchema', () => {
  test('rejects the reserved baseline contributor id', () => {
    const result = ServerModuleConfigSchema.safeParse({
      routes: {},
      routeContributions: {
        [SERVER_MODULE_BASELINE_ROUTE_OWNER]: { '/health': new Response('BAD') },
      },
    ...
    if (!result.success) {
      expect(result.error.issues[0]?.message).toContain(SERVER_MODULE_BASELINE_ROUTE_OWNER)
    }
  })
})
