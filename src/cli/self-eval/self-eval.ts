...
import { DEFAULT_K, DEFAULT_TIMEOUT } from './self-eval.constants.ts'
import type { Adapter, Grader, PromptCase, TrialEntry, TrialResult } from './self-eval.schemas.ts'
import { TrialResultSchema } from './self-eval.schemas.ts'
import {
  createWorkspaceDir,
  createWriteMutex,
  loadAdapter,
  ...
  const processPrompt = async (promptCase: PromptCase, index: number): Promise<TrialResult> => {
    ...
    for (let trialNum = 1; trialNum <= k; trialNum++) {
      ...
      try {
        ...
        const process = summarizeTrialProcess({
        ...
        const entry: TrialEntry = {
          ...
          duration,
          ...(adapterResult.trace && { trace: adapterResult.trace }),
          process,
        ...
        if (grader) {
          ...
          if (graderResult.metaVerification) {
            entry.metaVerification = graderResult.metaVerification
          }
        }
        ...
        const runnerError = isTimeout ? undefined : errorMessage

        entries.push({
          trialNum,
          output: '',
          duration,
          process: summarizeTrialProcess({
            timedOut: isTimeout,
            runnerError,
          }),
...
/**
 * CLI input schema for the self-eval command.
 *
 * @public
 */
export const SelfEvalInputSchema = z.object({
  ...
  verifierPath: z.string().optional().describe('Path to verifier script (meta-verification over grader output)'),
...
export const SelfEvalOutputSchema = z.array(TrialResultSchema)

/**
 * CLI handler for the self-eval command.
 *
 * @remarks
 ...
 * @public
 */
export const selfEvalCli = async (args: string[]): Promise<void> => {
  const input = await parseCli(args, SelfEvalInputSchema, {
    name: 'self-eval',
    outputSchema: SelfEvalOutputSchema,
  })
  ...
  const verifier = input.verifierPath ? await loadVerifier(input.verifierPath) : undefined
  const gradedWithVerification = grader && verifier ? withMetaVerification(grader, verifier) : grader
