...
import type { Adapter, Grader, GraderResult, PromptCase } from '../self-eval.schemas.ts'
import {
  GraderResultSchema,
...
import {
  ...
  resolvePath,
  runWorkerPool,
  summarizeTrialProcess,
  withMetaVerification,
} from '../self-eval.utils.ts'
...
/** Adapter that includes snapshot-native trace */
      ...
      snapshots: [
        {
          ...
          bids: [
            ...
            {
              thread: { label: 'guard', id: 'bt_2' },
              source: 'request',
              selected: false,
              type: 'task.start',
              detail: { text },
              priority: 2,
              blockedBy: { label: 'policy', id: 'bt_3' },
            },
          ],
        },
      ],
      selectedEvents: [{ type: 'task.start', detail: { text }, source: 'request', timestamp: Date.now() }],
      runtimeOutputs: [{ kind: 'tool_bash_result', status: 'ok', type: 'tool_bash_result', timestamp: Date.now() }],
    },
...
describe('runTrial', () => {
  test('k=1 single trial per prompt', async () => {
    ...
    expect(firstTrial!.process?.coverage).toBe('none')
    expect(firstTrial!.process?.selectionCount).toBe(0)
  })
  ...
  test('grader metaVerification is preserved on the trial entry', async () => {
    const verifierAwareGrader: Grader = async () => ({
      pass: true,
      score: 0.8,
      metaVerification: {
        confidence: 0.92,
        reasoning: 'Verifier agrees with grader output',
      },
    })

    const results = await runTrial({
      adapter: echoAdapter,
      prompts: [{ id: 'verified', input: 'test' }],
      grader: verifierAwareGrader,
    })
    ...
    const trial = result!.trials[0]
    expect(trial).toBeDefined()
    expect(trial!.metaVerification).toEqual({
      confidence: 0.92,
      reasoning: 'Verifier agrees with grader output',
    })
  })
  ...
  test('adapter failure records error entry', async () => {
    ...
    expect(trial!.process?.runnerErrorCount).toBe(1)
    expect(trial!.process?.runtimeErrorDetected).toBe(true)
  })
  ...
  test('adapter timeout records timed out entry', async () => {
    ...
    expect(trial!.pass).toBe(false)
    expect(trial!.process?.runnerTimeoutCount).toBe(1)
    expect(trial!.process?.runtimeErrorDetected).toBe(true)
  })

  test('rich adapter includes snapshot trace, process summary, and timing', async () => {
    const results = await runTrial({
      adapter: richAdapter,
    ...
    const trial = r!.trials[0]
    expect(trial).toBeDefined()
    expect(trial!.trace).toBeDefined()
    expect(trial!.trace!.snapshots).toHaveLength(1)
    expect(trial!.trace!.snapshots?.[0]?.kind).toBe('selection')
    expect(trial!.process).toBeDefined()
    expect(trial!.process!.selectionCount).toBe(1)
    expect(trial!.process!.selectedEventCount).toBe(1)
    expect(trial!.process!.blockedBidCount).toBe(1)
    expect(trial!.process!.runtimeErrorDetected).toBe(false)
    expect(trial!.timing).toBeDefined()
    expect(trial!.timing!.inputTokens).toBe(10)
    expect(trial!.timing!.outputTokens).toBe(20)
  })

  test('adapter runtime outputs are preserved on the trial trace', async () => {
    const outputTraceAdapter: Adapter = async ({ prompt }) => {
      const text = Array.isArray(prompt) ? prompt.join('\n') : prompt
      return {
        output: text,
        trace: {
    ...
    const results = await runTrial({
      adapter: outputTraceAdapter,
      prompts: [{ id: 'trace-trainable', input: 'test' }],
      grader: dimensionalGrader,
    })
    ...
    expect(result!.trials[0]?.trace?.runtimeOutputs?.[0]?.status).toBe('error')
    expect(result!.trials[0]?.process?.runtimeErrorDetected).toBe(true)
    expect(result!.trials[0]?.process?.runtimeOutputErrorCount).toBe(1)
  })

  test('grader receives snapshot-native trace and process summary', async () => {
    let seenSelectionCount = -1
    let seenCoverage = ''
    const traceAwareGrader: Grader = async ({ trace, process }) => {
      seenSelectionCount = trace?.snapshots?.filter((snapshot) => snapshot.kind === 'selection').length ?? 0
      seenCoverage = process?.coverage ?? ''
      return { pass: true, score: 1.0 }
    }

    await runTrial({
      adapter: richAdapter,
      prompts: [{ id: 'trace-grader', input: 'grade me' }],
      grader: traceAwareGrader,
    })

    expect(seenSelectionCount).toBe(1)
    expect(seenCoverage).toBe('snapshots-and-events')
  })
...
describe('CLI contract', () => {
  test('--schema input emits JSON Schema', () => {
    const schema = z.toJSONSchema(SelfEvalInputSchema)
    ...
    expect(props.verifierPath).toBeDefined()
  })

  test('--schema output emits JSON Schema', () => {
    const schema = z.toJSONSchema(SelfEvalOutputSchema)
    expect(schema.type).toBe('array')
  })

  test('SelfEvalInputSchema validates correct input', () => {
    const result = SelfEvalInputSchema.safeParse({
      adapterPath: './adapter.ts',
      promptsPath: './prompts.jsonl',
      k: 5,
      verifierPath: './verifier.ts',
    })
    expect(result.success).toBe(true)
  })

  test('SelfEvalInputSchema rejects missing adapterPath', () => {
    const result = SelfEvalInputSchema.safeParse({
      promptsPath: './prompts.jsonl',
    })
  ...
  test('SelfEvalInputSchema applies defaults', () => {
    const result = SelfEvalInputSchema.parse({
      adapterPath: './adapter.ts',
    })
...
describe('snapshot process helpers', () => {
  ...
  test('detects runtime errors from trace and runner context', () => {
  ...
  test('trace schema rejects non-JSON detail payloads', () => {
    const invalidTrace: unknown = {
      selectedEvents: [
        {
          type: 'task.invalid',
          detail: { fn: () => 'not json' },
    ...
    expect(() => PlaitedTraceSchema.parse(invalidTrace)).toThrow()
  })

  test('detects feedback errors and deadlocks', () => {
  ...
  test('counts selected events with explicit selectedEvents precedence', () => {
    expect(countSelectedEvents(trace)).toBe(2)
    expect(
      countSelectedEvents({
        snapshots: [
          {
  ...
  test('analyzes loop/repetition patterns', () => {
    expect(analyzeSelectionPatterns(trace)).toEqual({
      repeatedSelectionCount: 1,
      maxConsecutiveSelectionTypeCount: 2,
    })
  })

  test('counts blocked and interrupted bids', () => {
    expect(countBlockedBids(trace)).toBe(2)
    expect(countInterruptedBids(trace)).toBe(1)
  })

  test('detects trace coverage states', () => {
    expect(detectTraceCoverage()).toBe('none')
    expect(
      detectTraceCoverage({
        snapshots: [{ kind: 'runtime_error', error: 'x' }],
      }),
    ).toBe('snapshots-only')
    expect(
      detectTraceCoverage({
        emittedEvents: [{ type: 'event', detail: { ok: true } }],
      }),
    ).toBe('events-only')
    expect(detectTraceCoverage(trace)).toBe('snapshots-and-events')
  })

  test('summarizes trial process metrics', () => {
    const summary = summarizeTrialProcess({ trace, timedOut: true })
    expect(summary.coverage).toBe('snapshots-and-events')
    expect(summary.snapshotCount).toBe(5)
...
describe('TrialEntrySchema with dimensions', () => {
  ...
  test('accepts snapshot-native trace on TrialEntry', () => {
    const result = TrialEntrySchema.parse({
      trialNum: 1,
      output: 'ok',
      duration: 10,
    ...
    expect(result.trace!.snapshots?.[0]?.kind).toBe('runtime_error')
  })
})
...
describe('loadPolyglot', () => {
  test('loadAdapter from TS module', async () => {
    ...
    const { loadAdapter } = await import('../self-eval.utils.ts')
  ...
  test('loadGrader from TS module', async () => {
    ...
    const { loadGrader } = await import('../self-eval.utils.ts')
  ...
  test('loadVerifier from TS module', async () => {
    ...
    const { loadVerifier } = await import('../self-eval.utils.ts')
...
describe('package cli entrypoint exports self-eval surface', () => {
  test('re-exports self-eval schemas, runner, and utilities', async () => {
    const cli = await import('../../../cli.ts')

    expect(cli.SelfEvalInputSchema).toBeDefined()
    expect(cli.SelfEvalOutputSchema).toBeDefined()
    expect(typeof cli.runSelfEvalTrial).toBe('function')
    expect(cli.selfEvalSchemas).toBeDefined()
    expect(cli.selfEvalUtils).toBeDefined()
    expect(typeof cli.selfEvalUtils.withMetaVerification).toBe('function')
  })
})
