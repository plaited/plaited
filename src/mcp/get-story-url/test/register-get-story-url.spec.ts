...
test('registerGetStoryUrl registers tool correctly', () => {
  const server = getMcpServer()
  const domain = 'http://localhost:3000/'

  // Register the tool - should not throw on first registration
  expect(() => registerGetStoryUrl(server, domain)).not.toThrow()

  // Attempting to register again should throw (tool already registered)
  expect(() => registerGetStoryUrl(server, domain)).toThrow('Tool get-story-url is already registered')
})
