...
export type StoryExportDetail = {
  name: string
  type: 'interaction' | 'snapshot' | 'unknown'
  hasPlay: boolean
  hasArgs: boolean
  hasTemplate: boolean
 ...
 * ```
 */
export const getStorySetExportDetails = ({ filePath }: GetTemplateExportsParams): StoryExportDetail[] => {
  ...
  const storyExportDetails: StoryExportDetail[] = []

    /**
     * Analyzes a node to check for actual properties in object literal
     */
    const analyzeObjectLiteral = (node: ts.Node): Partial<StoryExportDetail> | null => {
     ...
     */
    const analyzeStoryType = (type: ts.Type, node?: ts.Node): Partial<StoryExportDetail> | null => {
      const typeString = typeChecker.typeToString(type)
      
      // Check if it's a StoryObj type
      if (!typeString.includes('StoryObj') && 
        ...
        if (type.symbol) {
          ...
          const hasStoryProps = propNames.some(name => 
            ['play', 'args', 'parameters', 'template'].includes(name)
          )
    ...
    const visit = (node: ts.Node): void => {
      ...
      // Check export assignments for default exports
      if (ts.isExportAssignment(node) && !node.isExportEquals) {
        // Check if it's an identifier (reference to a const)
        if (ts.isIdentifier(node.expression)) {
          const symbol = typeChecker.getSymbolAtLocation(node.expression)
          if (symbol && symbol.valueDeclaration) {
            const type = typeChecker.getTypeAtLocation(node.expression)
            const details = analyzeStoryType(type, symbol.valueDeclaration)
            if (details) {
              storyExportDetails.push({
                name: 'default',
                ...details,
          ...
          const type = typeChecker.getTypeAtLocation(node.expression)
          const details = analyzeStoryType(type, node)
          if (details) {
            storyExportDetails.push({
              name: 'default',
              ...details,
