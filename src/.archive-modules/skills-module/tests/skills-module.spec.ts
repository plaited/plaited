...
import type { SkillsCatalogSchema, SkillsSelectedSignalSchema } from '../skills-module.schemas.ts'
import { createSkillsModule } from '../skills-module.ts'
...
describe('createSkillsModule', () => {
  test('discovers valid skills into the catalog signal and emits an update event', async () => {
    const workspace = await mkdtemp(join(tmpdir(), 'plaited-skills-module-'))
...
`,
    ...
    const agent = await createAgent({
      ...
      modules: [
        ...
        ({ signals }) => {
          catalogSignal = signals.get(SKILLS_MODULE_SIGNAL_KEYS.catalog) as Signal<typeof SkillsCatalogSchema>
          catalogSignal.listen(() => resolveCatalog())

          return {
            handlers: {
              [SKILLS_MODULE_EVENTS.skills_module_catalog_updated](detail) {
                const parsed = z
                  .object({
                    count: z.number(),
    ...
    agent.trigger({ type: SKILLS_MODULE_EVENTS.skills_module_reload })
    await Bun.sleep(25)
...
`,
    ...
    const agent = await createAgent({
      ...
      modules: [
        ...
        ({ signals }) => {
          catalogSignal = signals.get(SKILLS_MODULE_SIGNAL_KEYS.catalog) as Signal<typeof SkillsCatalogSchema>
          selectedSignal = signals.get(SKILLS_MODULE_SIGNAL_KEYS.selected) as Signal<typeof SkillsSelectedSignalSchema>
          catalogSignal.listen(() => resolveCatalog())
