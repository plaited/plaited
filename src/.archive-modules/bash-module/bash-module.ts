...
} from './bash-module.schemas.ts'
import type { CreateBashModuleOptions } from './bash-module.types.ts'
...
/**
 * Creates the bounded bash-execution policy module.
 *
 * @remarks
 ...
 * @public
 */
export const createBashModule =
  ({ stateSignalKey = BASH_MODULE_SIGNAL_KEYS.state }: CreateBashModuleOptions = {}): Module =>
  ({ signals, trigger }) => {
    const stateSignal =
    ...
    const publish = (state: BashExecutionState) => {
      ...
      trigger({
        type: BASH_MODULE_EVENTS.bash_module_updated,
        detail: {
    ...
    return {
      handlers: {
        [BASH_MODULE_EVENTS.bash_module_request](detail) {
        ...
        [BASH_MODULE_EVENTS.bash_module_mark_result](detail) {
