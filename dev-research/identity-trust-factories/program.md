...
optional SSI-backed trust services should participate in the factory-composed
...
The target is not one mandatory standards stack. The target is a factory-owned
...
- `identity-trust-factories` owns stable node identity, peer trust semantics,
  and trust-service integration
- `node-auth-factories` owns local user/platform auth mode
- `a2a-factories` owns the communication protocol and Agent Card mechanics
- `node-discovery-factories` owns publication and rebinding policy
...
The first shipped identity/trust factory bundle should support:
...
- candidate identity/trust factory bundles
