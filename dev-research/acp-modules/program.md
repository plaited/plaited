...
- module-owned orchestration and policy
...
- inspecting capabilities and active module-owned surfaces
...
This lane should treat ACP as a module-owned operator interface layered on top
...
The first shipped ACP module should support:
...
This module is not primarily about multi-tenant hosting or public internet
...
- ACP terminates at a module boundary
- the ACP module translates ACP requests into local events and signals
- local modules still own planning, memory, skills, MCP, editing,
...
- the first ACP module should optimize for development/debugging ergonomics,
...
The ACP module should advertise what the local bundle can actually do, for
example:
...
A first ACP module should translate inbound ACP turns into local events such
as:
...
The exact vocabulary can be ACP-module-owned, but execution still needs to
compose through `createAgent()` and installed modules.
...
Candidate ACP modules should be judged on:
...
- a recommended ACP module shape for local operator control
...
- a recommendation about whether ACP belongs in the default module bundle
