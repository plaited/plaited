...
Research the default module bundle for bounded multi-attempt execution,
...
- `fanout-modules` owns bounded attempt creation, retention, comparison, and
  winner selection
- `workflow-state-modules` owns in-process orchestration of roles
- `plan-modules` owns decomposition and replan policy
- `observability-modules` owns shared artifact and trace conventions
...
The first shipped fanout module bundle should support:
...
- candidate fanout module bundles
