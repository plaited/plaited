...
Research the smallest effective module architecture around the minimal
...
The point is to hill-climb toward the right default module bundle around a
...
What remains unknown is which behaviors should become default modules around
...
set of composable modules that make that primitive reliable enough for the
default agent.

In other words:

- core should stay small
- modules should absorb policy, routing, validation, normalization, and
  observability
- the default shipped agent should emerge from judged module compositions
...
- assume the first reasonable module split is the final default
...
- what is the smallest module stack that makes `bash`-style execution usable
  for the default agent?
- which concerns belong in separate modules versus one bundled execution
  module?
...
- what result shape is sufficient for other modules to build on?
...
A module bundle where execution stays simple because workers must be declared
in a registry with:
...
A module bundle where each worker run must declare intended effects before the
actual run.
...
A module bundle where raw worker output is treated as too weak for a useful
default agent, so modules normalize results into typed runtime signals.
...
- the fastest gain comes from making execution results legible to later
  modules rather than from adding more validation first

### 4. Artifact Observability First

A module bundle where the main missing piece is durable observability around
worker runs.
...
A module bundle where the critical distinction is not specific workers but
...
This lane should evaluate candidate module bundles against tasks such as:
...
- amount of policy added to core versus modules
...
- one or more candidate module bundles around the minimal `bash` primitive
...
- a recommendation for which module bundle should become the default shipped
  behavior
- explicit notes on what must remain in core versus what should stay module-owned
...
- composes cleanly through the existing module contract
...
## Relation To Other Module Lanes

This lane is a focused subprogram under the broader default-module research
structure.
