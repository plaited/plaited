# Slice 2E: Parallel Data Collection - Worker A

## Target

Run 375 autoresearch attempts on worker thread E, generating high-quality Plaited-native code for distillation training data.

**Worker A runs independently on feature branch `native-model-worker-e`.**
...
- Feature branch: `native-model-worker-e`
...
- Committed to `native-model-worker-e` with full provenance
...
```bash
bun run research:overnight -- ./dev-research/native-model/slice-2e.md \
