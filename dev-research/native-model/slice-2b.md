# Slice 2B: Parallel Data Collection - Worker A

## Target

Run 375 autoresearch attempts on worker thread B, generating high-quality Plaited-native code for distillation training data.

**Worker A runs independently on feature branch `native-model-worker-b`.**
...
- Feature branch: `native-model-worker-b`
...
- Committed to `native-model-worker-b` with full provenance
...
```bash
bun run research:overnight -- ./dev-research/native-model/slice-2b.md \
