# Slice 2H: Parallel Data Collection - Worker A

## Target

Run 375 autoresearch attempts on worker thread H, generating high-quality Plaited-native code for distillation training data.

**Worker A runs independently on feature branch `native-model-worker-h`.**
...
- Feature branch: `native-model-worker-h`
...
- Committed to `native-model-worker-h` with full provenance
...
```bash
bun run research:overnight -- ./dev-research/native-model/slice-2h.md \
