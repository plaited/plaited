# Slice 2A: Parallel Data Collection - Worker A

## Target

Run 375 autoresearch attempts on worker thread A, generating high-quality Plaited-native code for distillation training data.

**Worker A runs independently on feature branch `native-model-worker-a`.**
...
- Feature branch: `native-model-worker-a`
...
- Committed to `native-model-worker-a` with full provenance
