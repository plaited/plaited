# Slice 2F: Parallel Data Collection - Worker A

## Target

Run 375 autoresearch attempts on worker thread F, generating high-quality Plaited-native code for distillation training data.

**Worker A runs independently on feature branch `native-model-worker-f`.**
...
- Feature branch: `native-model-worker-f`
...
- Committed to `native-model-worker-f` with full provenance
...
```bash
bun run research:overnight -- ./dev-research/native-model/slice-2f.md \
