# Slice 2D: Parallel Data Collection - Worker A

## Target

Run 375 autoresearch attempts on worker thread D, generating high-quality Plaited-native code for distillation training data.

**Worker A runs independently on feature branch `native-model-worker-d`.**
...
- Feature branch: `native-model-worker-d`
...
- Committed to `native-model-worker-d` with full provenance
...
```bash
bun run research:overnight -- ./dev-research/native-model/slice-2d.md \
