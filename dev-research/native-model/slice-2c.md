# Slice 2C: Parallel Data Collection - Worker A

## Target

Run 375 autoresearch attempts on worker thread C, generating high-quality Plaited-native code for distillation training data.

**Worker A runs independently on feature branch `native-model-worker-c`.**
...
- Feature branch: `native-model-worker-c`
...
- Committed to `native-model-worker-c` with full provenance
...
```bash
bun run research:overnight -- ./dev-research/native-model/slice-2c.md \
