...
Research the default notification-oriented factory bundle for the Plaited
agent.
...
The target is a factory family for meaningful attention routing, not a generic
stream of chatter.
...
either remain underspecified or leak into unrelated factories.
...
- `notification-factories` owns attention policy and notification shaping
- `projection-factories` owns context-facing summaries
- `observability-factories` owns richer retained traces and artifacts
...
attention policy rather than by ad hoc string emission from every factory.
...
The first shipped notification factory bundle should support:
...
- candidate notification factory bundles
