...
Research a factory family that explicitly governs the three-axis risk model for
...
This lane should supplement the other default-factory programs by covering the
...
factories without one program integrating them.
...
The three-axis model should be implemented as a composed factory layer, not
buried inside each individual capability factory.

That means:

- capability factories expose possible actions
- three-axis factories decide the conditions under which those actions are
...
- cross-cutting risk/control factories

## Product Target

The first shipped three-axis factory bundle should support:

1. classifying built-in and factory-provided capabilities by risk profile
...
6. remaining compatible with the minimal core and installed factory bundle
...
an implicit side effect of whichever factories happen to be installed.
...
- should capability registration be centralized or emitted by each factory into
  a shared control surface?
- what confirmation rules should apply by default to bash, MCP, and A2A?
- how should long-running autonomy be bounded when heartbeat-driven factories
  are installed?
...
- does it avoid duplicating control logic across capability factories?
...
- a proposed three-axis factory bundle shape
