...
Research the default projection-oriented factory bundle for the Plaited agent.
...
hoc summaries from each neighboring factory family.
...
- `projection-factories` owns live-context shaping and concise projections
- `observability-factories` owns richer retained evidence
- `memory-factories` owns long-horizon consolidation and recall
...
The default agent will use its other factories more reliably if internal state
...
The first shipped projection factory bundle should support:
...
- candidate projection factory bundles
