...
Define the first default module for the Plaited agent:

- [src/modules/skills-module/skills-module.ts](../../src/modules/skills-module/skills-module.ts)

This module should load local skills from the workspace, expose enough skill
...
current minimal-core + behavioral-module architecture.

## Why This Lane Exists

The current agent core already provides:

- runtime module installation via `update_modules`
...
This is a natural first default module because it gives the agent a policy
...
It is expected to inform the default module bundle decision, but it should
...
to `search-modules`, not to this lane alone.
...
- [src/modules/skills-module/skills-module.ts](../../src/modules/skills-module/skills-module.ts)
- [src/modules/skills-module/skills-module.utils.ts](../../src/modules/skills-module/skills-module.utils.ts)
...
- `SkillFrontMatterSchema` in [src/modules/skills-module/skills-module.utils.ts](../../src/modules/skills-module/skills-module.utils.ts)
...
The best skill-module design should follow this split:

- `createAgent()` remains the execution boundary
- the skill module owns skill discovery, selection policy, and context
  assembly
...
- skills should become agent behavior through modules

## Product Target

The first shipped skill module should support this end-to-end flow:
...
The module should likely introduce events for moments such as:
...
The skill module should not add its own bespoke file execution stack.
...
the module should route through the existing core events and handlers where
possible:
...
The model can choose a skill, but the module should own:
...
The first module candidate should likely own:

- startup discovery of local skills
- re-discovery when modules or relevant workspace content change
...
- whether skill selection is represented as a direct model tool call, a tagged
  message convention, or a BP event emitted by another module
...
The module itself and its live signal objects should remain type-first.
...
- is the resulting module understandable enough to ship by default
...
- the executable module at [src/modules/skills-module/skills-module.ts](../../src/modules/skills-module/skills-module.ts)
- any supporting schemas, constants, and tests under
  [src/modules/skills-module/](../../src/modules/skills-module)
- retained notes on which `example-agent` behaviors were adopted or reshaped
- a clear recommendation on whether this module should be included in the
