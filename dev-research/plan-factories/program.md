...
Research the default planning-oriented factory bundle for the Plaited agent.
...
The target is not a giant built-in planner. The target is a factory family
...
execution factories, and fully owned by none of them.
...
- `plan-factories` owns decomposition, phase transitions, and plan-state policy
- `edit-factories` owns concrete change execution policy
- `verification-factories` owns checks and repair triggers
- `projection-factories` owns how plan state is exposed back to the model
...
The first shipped planning factory bundle should support:
...
### 1. Planning Is A Factory Surface
...
- candidate planning factory bundles
