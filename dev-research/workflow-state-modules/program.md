...
Research a module family that captures team-like orchestration inside one
Plaited agent runtime without spawning subagents as the primary abstraction.

This lane should define how the agent:

- installs multiple role-specialized module layers inside one sovereign node
...
- supports both startup-installed defaults and runtime-installed dynamic
  overlays through `AGENT_EVENTS.update_modules`
...
in-process orchestration model built from modules.

## Why This Lane Exists

The repo already has the core execution seam needed for this direction:

- [src/agent/create-agent.ts](../../src/agent/create-agent.ts) installs a `modules` array at init time
...
- when a role should be a startup-installed module versus a runtime overlay
...
- when a module family should behave like a transient "teammate" without
...
through the module model instead of copying teammate spawning.
...
- `workflow-state-modules` owns team-like in-process orchestration patterns
- `plan-modules` owns decomposition and plan-state policy
- `edit-modules` owns concrete change execution policy
- `verification-modules` owns checks, blocking, and repair routing
- `module-discovery-modules` owns how runtime module modules are found and
  loaded

## Dependency Order

1. [src/agent/create-agent.ts](../../src/agent/create-agent.ts) defines init-time and runtime module
   installation
...
cheaply and more coherently as multiple modules sharing one event space.
...
- subagent lifecycle becomes module install, activation, interruption, and
  teardown policy

## Product Target

The first shipped workflow-state module bundle should support:
...
- planner, editor, verifier, and projector modules are installed once at
  startup
...
A small static kernel ships at startup, then phase-specific module modules are
loaded later through `AGENT_EVENTS.update_modules`.
...
Modules are loaded dynamically one or more times, with each install creating
an isolated role instance inside the shared runtime.

Pattern:

- `update_modules` installs the same module multiple times with different
...
- is the difference between static and dynamic module installation explicit?
- can repeated `update_modules` installs remain understandable and safe?
...
- when is process-level isolation still justified instead of module
  composition?
...
- candidate workflow-state module bundles
