...
Research a module family that owns agentic payment behavior for the default
Plaited agent.
...
is a composed module layer around paid boundaries such as x402 and MPP.
...
- `payment-modules` owns payment discovery, negotiation, spend-state policy,
  and payment evidence handling
- `three-axis-modules` owns cross-cutting approval and authority shaping
- `node-auth-modules` owns authenticated identity and trust context
- `verification-modules` owns correctness checks and repair routing
- `mcp-modules` and `a2a-modules` own remote transport and capability
  projection
...
3. [dev-research/three-axis-modules/program.md](../three-axis-modules/program.md) defines capability,
   autonomy, and authority control
4. [dev-research/node-auth-modules/program.md](../node-auth-modules/program.md) defines trust and exposure
   shaping
...
Those facts are enough to justify a dedicated module lane even if Plaited
...
The best default design will treat payment as a module-owned policy layer
...
- capability modules may expose paid and unpaid actions
- payment modules decide how paid actions are discovered, negotiated, and
  evidenced
...
The first shipped payment module bundle should support:
...
- what is the smallest useful payment module bundle worth shipping?
- should payment discovery be its own module or part of remote capability
  projection?
...
- candidate payment module bundles
