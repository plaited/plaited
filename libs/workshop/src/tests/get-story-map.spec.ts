...
test('getStoryMap: bad-meta', async () => {
  ...
  expect(logMessages.some(el => el.endsWith('is missing key values pairs'))).toBeTrue()
  expect(logMessages.some(el => el.startsWith('Export: [ default ] '))).toBeTrue()
...
test('getStoryMap: bad-title', async () => {
  ...
  expect(logMessages[0].startsWith('Invalid title')).toBeTrue()
  expect(storyMap.size).toBe(0)
})
...
test('getStoryMap: duplicate-title', async () => {
  ...
  const storyMap = await getStoryMap(entryPoints, srcDir)
  expect(logMessages.length).toBe(1)
  expect(logMessages[0].startsWith('Rename meta')).toBeTrue()
...
test('getStoryMap: bad-story', async () => {
  ...
  expect(logMessages[0].startsWith('Invalid name')).toBeTrue()
  expect(logMessages[1].startsWith('Exported Story: [ ')).toBeTrue()
  expect(logMessages[2].endsWith('missing key values pairs')).toBeTrue()
  expect(storyMap.size).toBe(0)
})
...
test('getStoryMap: duplicate-stories', async () => {
  ...
  const storyMap = await getStoryMap(entryPoints, srcDir)
  expect(logMessages.length).toBe(1)
  expect(logMessages[0].startsWith('Rename story:')).toBeTrue()
