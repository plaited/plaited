import { Formatter, DefaultToken, AngleToken, AmountToken } from '../../types.js'
...
export const defaultFormatter: Formatter<DefaultToken | AngleToken | AmountToken> = (
  token,
  ...
  if (isStaticToken(token)) {
    ...
    return getRule({ prop, value: hasAlias($value)
      ...
      : Array.isArray($value)
      ? $value.map(val => hasAlias(val) ? resolveCSSVar(val, allTokens) : val).join(hasCommaSeparatedValue(token) ? ', ' : ' ')
      : $value
    })
  }
  const toRet: string[] = []
      ...
      if (isValidContext({ ctx, contexts })) {
        toRet.push(
          getRule({
            ...
            ? contextValue.map(val => hasAlias(val) ? resolveCSSVar(val, allTokens) : val).join(hasCommaSeparatedValue(token) ? ', ' : ' ')
            : contextValue,
            ctx,
            contexts,
          }),
