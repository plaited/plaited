...
The client-side `behavioral()` shell coordinates all rendering, user input, and interactive element lifecycle.
...
### Thread Region Tracker

**`src/ui/thread-region-tracker.ts`**

Tracks which agent-generated behavioral threads belong to which DOM region. When a region is re-rendered, its threads are cleaned up via `bThreads.delete()`.
