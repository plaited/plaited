...
describe('modnet-prompt-derivation-meta-verifier', () => {
  ...
  test('builds prompt with continuity risks and meta-signal checks', () => {
    ...
    expect(prompt).toContain('approved parent context')
    expect(prompt).toContain('source-anchor signals')
    expect(prompt).toContain('Precursor-chain assertions')
    expect(prompt).toContain('chain context')
    expect(prompt).toContain('source precursor shape')
