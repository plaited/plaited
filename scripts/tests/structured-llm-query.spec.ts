...
describe('structured-llm-query', () => {
  ...
  test('coerces string-typed hypergraph search arguments from tool calls', async () => {
    const root = await mkdtemp(join(tmpdir(), 'plaited-structured-query-graph-coerce-'))
    const graphDir = join(root, 'dev-research', 'behavioral-seed', 'seed')
    await Bun.$`mkdir -p ${graphDir}`.quiet()
    await Bun.write(
      join(graphDir, 'anchors.jsonld'),
      JSON.stringify({
        '@id': 'behavioral:seed',
        '@type': 'BehavioralSeed',
        provides: [{ '@id': 'behavioral:anchor/behavioral', '@type': 'Anchor' }],
      }),
    )
    ...
    globalThis.fetch = (async (_input: RequestInfo | URL, init?: RequestInit) => {
      const bodyText = typeof init?.body === 'string' ? init.body : ''
      if (!bodyText.includes('"role":"tool"')) {
        return new Response(
          JSON.stringify({
            choices: [
              {
                  ...
                  tool_calls: [
                    {
                      ...
                      function: {
                        ...
                        arguments: JSON.stringify({
                          path: 'dev-research/behavioral-seed/seed',
                          query: 'reachability',
                          startVertices: 'behavioral:anchor/behavioral',
                          maxDepth: '2',
                        }),
                      },
                    },
      ...
      return new Response(
        JSON.stringify({
          choices: [
            {
              message: {
                content: JSON.stringify({ verdict: 'graph-coerced' }),
              },
            },
          ],
        }),
    ...
    try {
      ...
      expect(result.ok).toBe(true)
      if (result.ok) {
        expect(result.value.verdict).toBe('graph-coerced')
        expect(result.meta?.toolRounds).toBe(1)
      }
    } finally {
      globalThis.fetch = originalFetch
