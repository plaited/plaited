...
import { BEHAVIORAL_CORPUS_JUDGE_CRITERIA } from './behavioral-corpus-grader.ts'
import { resolveMetaVerifierModel, runStructuredLlmQuery } from './structured-llm-query.ts'

export const getBehavioralCorpusJudgeInput = (result: GraderResult): WorkspaceImprovementJudgeInput | null => {
...
export const verify: Verifier = async (result: GraderResult) => {
  const input = getBehavioralCorpusJudgeInput(result)
  const workspaceRoot =
