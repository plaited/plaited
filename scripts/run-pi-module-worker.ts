...
const parseArgs = (argv: string[]): WorkerArgs => {
  ...
  if (!programPath || !artifactDir || !planPath) {
    throw new Error('Usage: bun scripts/run-pi-module-worker.ts --program <path> --artifact-dir <dir> --plan <path>')
  }
...
export const buildPiWorkerPrompt = ({
  ...
  return [
    ...
    'After the first substantive edit, run the narrow formatting, type checking, and targeted test commands for the affected lane surface. Use those failures to guide the remaining edits instead of deferring validation until the end.',
    'Before finishing, make sure the generated code passes formatting, type checking, and targeted tests for the affected files or lane surface. If validation fails on mechanical issues such as lint, formatting, imports, or unused variables, fix them before stopping.',
    'Do not claim validation succeeded unless you actually ran the commands in this worktree and saw them pass. Do not run an unrelated full test suite.',
