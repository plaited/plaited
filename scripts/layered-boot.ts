 ...
 */
// Use the project root's tsc binary and @types — generated workspaces don't install TypeScript
const PROJECT_ROOT = `${import.meta.dir}/..`
const TSC_BIN = `${PROJECT_ROOT}/node_modules/.bin/tsc`
const TYPES_NODE_DIR = `${PROJECT_ROOT}/node_modules/@types/node`

/**
 * Minimal tsconfig written to each workspace for Layer 2 tsc checks.
 * Points typeRoots at the project root's @types so Node.js globals resolve.
 */
const buildTsconfig = (nodeModulesDir: string): string =>
  JSON.stringify({
    compilerOptions: {
      target: 'ESNext',
      module: 'ESNext',
      ...
      strict: true,
      // Override strict's noImplicitAny — generated modules iterate over any-typed
      // external API data (Bluesky, Leaflet, etc.), so implicit any in callbacks is
      // expected noise rather than a logic error. All other strict checks still apply.
      noImplicitAny: false,
      noEmit: true,
      skipLibCheck: true,
      allowJs: true,
      typeRoots: [`${nodeModulesDir}/@types`],
      types: ['node', 'bun'],
      // Map 'plaited' and '@plaited/*' to the project root's source files.
      // h/Fragment are in jsx-runtime; createSSR is in ui/render/ssr.
      // Also map common typo '@plaited/temple' (generated code variant).
      paths: {
        // Eval stubs that re-export h, Fragment, createSSR in one place
        plaited: [`${PROJECT_ROOT}/scripts/eval-stubs/plaited.ts`],
        'plaited/ssr': [`${PROJECT_ROOT}/src/ui/render/ssr.ts`],
        '@plaited/template': [`${PROJECT_ROOT}/scripts/eval-stubs/plaited.ts`],
        '@plaited/template/ssr': [`${PROJECT_ROOT}/src/ui/render/ssr.ts`],
        // Common typo variant '@plaited/temple' generated by some agents
        '@plaited/temple': [`${PROJECT_ROOT}/scripts/eval-stubs/plaited.ts`],
        '@plaited/temple/ssr': [`${PROJECT_ROOT}/src/ui/render/ssr.ts`],
      },
    },
    exclude: ['node_modules', 'tsconfig.eval.json'],
  }, null, 2)
...
const checkLayer2 = async (workspace: string): Promise<LayerResult> => {
  ...
  // Symlink project root's node_modules into workspace so imports resolve
  // (eval expects TypeScript to compile, not that packages are installed)
  const wsNodeModules = join(workspace, 'node_modules')
  const wsNodeModulesFile = Bun.file(wsNodeModules + '/.placeholder')
  if (!(await wsNodeModulesFile.exists())) {
    await Bun.$`ln -sf ${PROJECT_ROOT}/node_modules ${wsNodeModules}`.quiet().nothrow()
  }

  // Copy ambient module stubs for packages not in project root (e.g., @atproto/api, leaflet)
  await Bun.$`cp ${PROJECT_ROOT}/scripts/eval-stubs/ambient-modules.d.ts ${workspace}/ambient-modules.d.ts`.quiet().nothrow()

  // Write a temporary tsconfig pointing at the project root's @types
  const tsconfigPath = join(workspace, 'tsconfig.eval.json')
  await Bun.write(tsconfigPath, buildTsconfig(wsNodeModules))

  const result = await Bun.$`${TSC_BIN} --project tsconfig.eval.json`
    .cwd(workspace)
    .nothrow()
    .quiet()

  // tsc writes errors to stdout; stderr has tool metadata
  const tscResult = { exitCode: result.exitCode, output: result.stdout.toString() + result.stderr.toString() }

  if (tscResult.exitCode !== 0 && tscResult.exitCode !== null) {
    const errors = tscResult.output.slice(0, 400)
