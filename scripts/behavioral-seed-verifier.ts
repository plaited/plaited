...
import { BEHAVIORAL_SEED_JUDGE_CRITERIA } from './behavioral-seed-grader.ts'
import { resolveMetaVerifierModel, runStructuredLlmQuery } from './structured-llm-query.ts'

export const getBehavioralSeedJudgeInput = (result: GraderResult): WorkspaceImprovementJudgeInput | null => {
...
export const verify: Verifier = async (result: GraderResult) => {
  const input = getBehavioralSeedJudgeInput(result)
  const workspaceRoot =
