...
const DEFAULT_PROGRAM_PHASES: ProgramPhase[] = [
  {
    ...
    programs: [
      ...
      'dev-research/fanout-modules/program.md',
      'dev-research/verification-modules/program.md',
      'dev-research/observability-modules/program.md',
    ],
  },
  {
    name: 'behavior',
    programs: [
      'dev-research/plan-modules/program.md',
      'dev-research/edit-modules/program.md',
      'dev-research/context-assembly-modules/program.md',
      'dev-research/tool-registry-modules/program.md',
    ],
  },
  {
    name: 'capabilities',
    programs: [
      'dev-research/search-modules/program.md',
      'dev-research/memory-modules/program.md',
      'dev-research/module-discovery-modules/program.md',
      'dev-research/skill-modules/program.md',
      'dev-research/mcp-modules/program.md',
      'dev-research/a2a-modules/program.md',
    ],
  },
  {
    name: 'policy',
    programs: [
      ...
      'dev-research/identity-trust-modules/program.md',
      'dev-research/permission-audit-modules/program.md',
      'dev-research/notification-modules/program.md',
    ],
  },
  {
    name: 'integration',
    programs: [
      'dev-research/default-modules/program.md',
      'dev-research/agent-bootstrap/program.md',
      'dev-research/autoresearch-modules/program.md',
    ],
  },
]
...
const buildProgramRunDir = ({ programPath, workspaceRoot }: { programPath: string; workspaceRoot: string }) =>
  resolve(workspaceRoot, '.worktrees', 'module-program-runner', basename(dirname(programPath)), timestamp())
...
const buildPlannerPrompt = async ({ programPath }: { programPath: string }) => {
  ...
  return [
    'Read AGENTS.md and the provided module-program lane.',
...
const createPlannedRunInput = ({
  ...
  workerCommand: [
    'bun',
    'scripts/run-pi-module-worker.ts',
    '--program',
    '{{program}}',
    '--artifact-dir',
    '{{artifact_dir}}',
...
const runPreflight = async ({ workspaceRoot }: { workspaceRoot: string }): Promise<PreflightCheck[]> => {
  ...
  const commands: Array<{ name: string; args: string[] }> = [
    ...
    { name: 'script-tests', args: ['bun', 'test', 'scripts/tests/module-program-scripts.spec.ts'] },
  ]
...
export const buildPromotionPrompt = async ({
  ...
  return [
    ...
    'You are the final promotion gate for an autonomous module-program run.',
...
const runProgramLane = async ({
  ...
  const judgeArtifact = await callOpenRouterReview({
    ...
    system:
      'You are a strict judge for autonomous module-program attempts. Return JSON only with keys decision and reasoning. decision must be exactly one of: accept, defer, reject.',
...
const main = async () => {
  ...
  const outputDir = resolve(
    input.outputDir ?? join(workspaceRoot, '.plaited', 'orchestration', 'module-programs', timestamp()),
  )
