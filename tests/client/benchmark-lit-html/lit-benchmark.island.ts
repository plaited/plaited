...
import { html, render } from 'https://esm.sh/lit-html@2.7.0'
...
export const LitBenchmark = isle(
  { tag: 'lit-benchmark-island' },
  (base) =>
    class extends base {
      plait({ feedback, addThreads, loop, sync, $, trigger }: PlaitProps) {
        ...
        feedback({
          ...
          render() {
            const data = getData()
            const [tbody] = $('tbody')
            render(TableBodyTemplate({ data }), tbody as HTMLElement)
