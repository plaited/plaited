...
const requestInParameter = ({ type: requestEventName, detail: requestDetail = {} }) => {
    return ({ type: parameterEventName, cb: parameterAssertion, }) => (parameterAssertion
        ? parameterAssertion({
...
dev, } = {}) => {
    ...
    function selectNextEvent() {
        ...
        for (const { request, priority } of bids) {
            if (Array.isArray(request)) {
                candidates = candidates.concat(request.map(event => ({ priority, ...event }) // create candidates for each request with current bids priority
                ));
                continue;
            }
            if (request) {
        ...
        const filteredBids = candidates.filter(request => !blocked.some(requestInParameter(request)));
    ...
    const feedback = actions => {
        actionPublisher.subscribe((data) => {
    ...
    if (dev && snapshotPublisher) {
        snapshotPublisher.subscribe((data) => dev(data));
    }
