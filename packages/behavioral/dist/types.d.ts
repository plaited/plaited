...
export type Trigger = <T extends (Detail) = (Detail)>(
  args: TriggerArgs<T>,
) => void;
...
type Callback<T extends (Detail) = (Detail)> = (args: {
  type: string;
  detail: T;
}) => boolean;
...
export type RulesFunc<T extends (Detail) = (Detail)> = () => IterableIterator<
  RuleSet<T>
>;
...
type Actions<T extends Record<string, (detail: Detail) => void>> = {
  [K in keyof T]: T[K] extends (detail: infer D) => void
    ? (detail: D extends Detail ? D : (Detail)) => void
    : never;
};
