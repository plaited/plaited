...
const joinParts = (tag, attrs = [], children) => `<${[tag, ...attrs].join(' ')}>${children.join('')}</${tag}>`;
/** createTemplate function used for ssr */
export const createTemplate = (tag, attrs) => {
    const { shadowrootmode = 'open', children: _children, shadowrootdelegatesfocus = true, trusted, slots: _slots, stylesheet, style, key: _, 'data-trigger': trigger, ...attributes } = attrs;
    ...
    if (trigger) {
        const value = Object.entries(trigger).map(([ev, req]) => `${ev}->${req}`)
            .join(' ');
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (isCustomElement) {
            templateChildren.push(trusted ? `${formattedChild}` : escape(`${formattedChild}`));
            continue;
        }
        /** P5 child IS {@type Primitive} */
        rootChildren.push(trusted ? `${formattedChild}` : escape(`${formattedChild}`));
    }
    if (isCustomElement) {
        ...
        if (stylesheets.size) {
            templateChildren.unshift(joinParts('style', undefined, [...stylesheets]));
        }
         ...
         */
        rootChildren.unshift(joinParts('template', templateAttrs, templateChildren));
        stylesheets.clear();
