...
test.after(async () => {
  await fs.rm(path.resolve(target, './sum'), { recursive: true })
  await fs.writeFile(path.resolve(target, './index.ts'), "export * from 'mock/src/sum'\n")
})

test('eject()', async t => {
  const cliPath = `${__dirname}/eject-wrapper.js`
  const expected = "export * from './sum'\n"
  await run([ cliPath ], [ ENTER ])
  const actual1 = await fs.readFile(path.resolve(target, './index.ts'), { encoding: 'utf8' })
  t.is(actual1, expected)
  const actual2 = await getStat(path.resolve(target, './sum/index.ts'))
  t.truthy(actual2)
  await run([ cliPath ], [ ENTER, 'y', ENTER ])
  const actual3 = await getStat(path.resolve(target, './sum/index.ts'))
  t.truthy(actual3)
})
