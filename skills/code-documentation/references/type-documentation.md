...
1. **Analyze type structure** using the `lsp` tool:
   - Run `lsp` with a `symbols` operation to list all type definitions in a file
   - Run `lsp` with a `hover` operation on the type to get the full signature
...
2. **Find type relationships** using the `lsp` tool:
   - Run `lsp` with a `references` operation to find all usages of the type
   - Run `lsp` with a `find` operation to search for related types by name pattern
