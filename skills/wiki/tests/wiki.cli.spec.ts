...
const TYPESCRIPT_LSP_RUNNER_COMMAND = 'bun run skills/typescript-lsp/scripts/typescript-lsp.ts '
...
describe('wiki CLI', () => {
  test('context mode ranks relevant docs from supplied paths and excludes AGENTS.md and skills', async () => {
    ...
    try {
      ...
      const result = await Bun.$`bun run skills/wiki/scripts/wiki.ts ${JSON.stringify({
        mode: 'context',
        rootDir,
        paths: ['docs', 'skills'],
...
`,
  ...
  test('diagnose mode emits a runnable workspace-scan LSP suggestion for code paths', async () => {
    ...
    expect(payload.file).toBe('src/cli/cli.ts')
    expect(payload.files).toEqual(['src/cli/cli.ts'])
    expect(payload.operations).toEqual([{ type: 'workspace-scan' }])
    expect(payload.targets).toBeUndefined()

    const lspResult = await Bun.$`bun run skills/typescript-lsp/scripts/typescript-lsp.ts ${JSON.stringify(payload)}`
      .cwd(CLI_PACKAGE_ROOT)
      .quiet()
      .nothrow()
