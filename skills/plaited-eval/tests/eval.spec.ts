...
import { EvalModeSchema } from '../scripts/eval.schemas.ts'
...
const runEvalScript = async (args: string[]) =>
  Bun.$`bun run skills/plaited-eval/scripts/eval.ts ${args}`.cwd(CLI_PACKAGE_ROOT).quiet().nothrow()

const runEvalCommand = async (input: unknown) =>
  Bun.$`bun run skills/plaited-eval/scripts/eval.ts ${JSON.stringify(input)}`.cwd(CLI_PACKAGE_ROOT).quiet().nothrow()
...
describe('eval CLI', () => {
  ...
  test('eval script emits the eval input schema', async () => {
    const result = await runEvalScript(['--schema', 'input'])
