...
const runTypescriptLsp = async (input: unknown, extraArgs: string[] = []) => {
  const proc = Bun.spawn(
    ['bun', 'run', 'skills/typescript-lsp/scripts/typescript-lsp.ts', JSON.stringify(input), ...extraArgs],
    {
...
describe('typescript-lsp CLI registration and schema', () => {
  test('output schema includes data field for successful results', async () => {
    const result = await Bun.$`bun run skills/typescript-lsp/scripts/typescript-lsp.ts --schema output`
      .cwd(CLI_PACKAGE_ROOT)
      .quiet()
  ...
  test('find includes rootDir-relative path for symbol locations', async () => {
    ...
    expect(symbols.some((entry) => entry.location?.path === 'skills/typescript-lsp/tests/fixtures/sample.ts')).toBe(
      true,
    )
  })
})
