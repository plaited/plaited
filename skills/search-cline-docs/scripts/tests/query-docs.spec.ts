...
const SCRIPT_PATH = new URL('../query-docs.ts', import.meta.url).pathname

describe('search-cline-docs query-docs script', () => {
  ...
  test('exits 2 when command field is missing', async () => {
    ...
    const stderr = await new Response(proc.stderr).text()
    expect(exitCode).toBe(2)
    expect(stderr).toContain('Missing required field: command')
  })
})
