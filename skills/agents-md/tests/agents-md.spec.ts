...
const runAgentsMdScript = async (args: string[]) =>
  Bun.$`bun run skills/agents-md/scripts/agents-md.ts ${args}`.cwd(CLI_PACKAGE_ROOT).quiet().nothrow()

const runAgentsMdCommand = async (input: unknown) =>
  Bun.$`bun run skills/agents-md/scripts/agents-md.ts ${JSON.stringify(input)}`.cwd(CLI_PACKAGE_ROOT).quiet().nothrow()

describe('agents-md CLI', () => {
  test('agents-md script exposes input and output schemas', async () => {
    const inputResult = await runAgentsMdScript(['--schema', 'input'])
    ...
    const outputResult = await runAgentsMdScript(['--schema', 'output'])
