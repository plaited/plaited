...
```typescript
// Canonical source: src/improve.ts
...
The `withMetaVerification` wrapper (from `src/improve/trial.ts`) runs a grader multiple times and produces a `MetaVerification` result. During probing phase, this catches grader failures before they corrupt training signal:
