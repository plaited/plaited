...
import { KANBAN_MODES, WORK_ITEM_LIFECYCLE_STATES } from '../scripts/kanban.constants.ts'
...
const runKanbanCommand = async (input: unknown) =>
  Bun.$`bun run skills/plaited-kanban/scripts/kanban.ts ${JSON.stringify(input)}`
    .cwd(CLI_PACKAGE_ROOT)
    .quiet()
    .nothrow()

const runRawKanbanCommand = async (rawInput: string) =>
  Bun.$`bun run skills/plaited-kanban/scripts/kanban.ts ${rawInput}`.cwd(CLI_PACKAGE_ROOT).quiet().nothrow()
...
describe('kanban CLI', () => {
  test('schema exposes simplified ledger modes and omits old policy surfaces', async () => {
    const inputSchemaResult = await Bun.$`bun run skills/plaited-kanban/scripts/kanban.ts --schema input`
      .cwd(CLI_PACKAGE_ROOT)
      .quiet()
      .nothrow()
