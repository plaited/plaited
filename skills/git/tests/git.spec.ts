...
const runPlaitedGitCommand = async (input: unknown) =>
  Bun.$`bun run skills/git/scripts/git.ts ${JSON.stringify(input)}`.cwd(CLI_PACKAGE_ROOT).quiet().nothrow()
...
describe('git CLI', () => {
  ...
  test('git schemas expose mode and base in CLI contracts', async () => {
    const inputSchemaResult = await Bun.$`bun run skills/git/scripts/git.ts --schema input`
      .cwd(CLI_PACKAGE_ROOT)
      .quiet()
      .nothrow()
    const outputSchemaResult = await Bun.$`bun run skills/git/scripts/git.ts --schema output`
      .cwd(CLI_PACKAGE_ROOT)
      .quiet()
      .nothrow()
