...
- the earlier `src/factories` promotion step has now been rolled back
- the factory concepts remain preserved in:
  - skills
  - docs
  - research notes
- fresh default factories should be rebuilt later against the current
  `src/agent` core rather than preserving the stale runtime surface
...
The current preferred deployment model is:

- the personal agent operates inside a single workspace that is itself a git
  repo
- `modules/*` are workspace-local module directories inside that repo
- worktrees remain the active attempt boundary for isolated exploration
- OS-level backups provide catastrophic recovery outside Plaited runtime

Modules should not currently be treated as separate runtime nodes or separate
repos by default.

Instead:

- modules provide behavior to the agent primarily through exported factories
- `update_factories` is the runtime install boundary for those module exports
- a module runtime contract should be explicit and minimal:
  - `export default [factoryA, factoryB]`

This keeps the provenance model simple:

- one repo history
- worktree-backed isolated attempts
- commits as the durable promotion and provenance unit
- optional later indexing/search layers over commit, file, snapshot, and
  retained-artifact history

For autonomous operation, branches should not be part of the normal control
surface:

- the workspace repo is the main provenance boundary
...
Shipped default behavior should not be preserved as stale executable runtime
surface just to keep the concept alive.

For now:

- accepted factory ideas should stay preserved in skills, docs, and research
  notes
- the runtime should remain agent-first and minimal
- fresh default factories can be rebuilt later against the current core when
  the desired baseline is clearer
- module-facing runtime behavior should be provisioned through explicit factory
  exports rather than bespoke module runtimes
- the current preferred module contract is:
  - `export default [factoryA, factoryB]`
...
2. Decide which remaining `src/agent/*` files should move into
   `src/agent` capability helpers, bootstrap-owned composition, or later
   rebuilt default factory implementations.
...
### Default Factory Rebuild

The old `src/factories` surface should not be treated as current architecture.

The intended direction is:

- keep accepted factory ideas and decomposition in markdown and skills
- continue shrinking `src/agent` toward the minimal execution core
- rebuild fresh default factory implementations later once they align with the
  current core capability and signal model
- let modules participate through the same factory contract rather than a
  separate sub-agent or module-runtime layer
